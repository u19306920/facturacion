...
echo "</pre>";
*/
$Detalles = new Comprobantes_detalles();

//Agregamos y guardamos detalle de Factura
foreach ($_SESSION['items'] as $key) {
	//print_r($key);
	$Detalles->add($key['idordenes_detalles'],$key['cantidad'],$_SESSION['idcomprobantes']);
}

$objOrdenesF = new Ordenes();

foreach ($_SESSION['carrito'] as $idorden) {
  $objOrdenesF->cambiarOrdenxFactura($idorden['idordenes']);
}
